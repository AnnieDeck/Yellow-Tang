...
```{r}
# load packages
pacman::p_load(ggplot2, phyloseq, treemapify, tidyverse, dplyr, vegan, sigclust2, dendextend, pairwiseAdonis, patchwork, cowplot, microbiome)
...
##Taxonomy 
Taxonomy_table_ch3 = read.csv("taxonomy.csv", sep=",", row.names=1)
...
phylum_colors_custom <- c(
  "Proteobacteria" = "#9e9ac8",        # Light purple (dominant)
  "Bacteroidota" = "#c51b8a",          # Magenta/pink
  "Actinobacteriota" = "#fa9fb5",      # Light pink
  "Fusobacteriota" = "#238b45",        # Dark green
  "Firmicutes" = "#c7e9b4",            # Light green
  "Planctomycetota" = "#253494",       # Dark blue
  "Halobacterota" = "#99d8c9",         # Light teal/cyan
  "Nitrospirota" = "#02818a",          # Teal
  "Myxococcota" = "#c6dbef",           # Light blue
  "Crenarchaeota" = "#feb24c",         # Orange
  "Campilobacterota" = "#fdbb84",      # Light orange
  "Chloroflexi" = "#ef3b2c",           # Red/orange
  "Desulfobacterota" = "#cc4c02",      # Brown/orange
  "Bdellovibrionota" = "#ffffcc"       # Pale yellow
)
...
## Automatically get all unique genera and put "Other" last
...
ggsave("Figure_S1.pdf", plot = NMDS_figure_all, width = 10, height = 8, units = "in", dpi = 300)
...
```{r}
##starting with CLR transformed larvae only dataframe made in NMDS chunk above (CLR_larvae)
...
vibrio_filtered_all <- vibrio_filtered_all %>%
  ...
  mutate(Timepoint_dph = factor(Timepoint_dph,
                                 levels = c("2", "5", "8", "13",
                                           "20", "36", "55", "76")))
...
vibrio_filtered_all <- vibrio_filtered_all %>%
  mutate(SampleType = case_when(
    ...
    TRUE ~ SampleType  # keeps other sample types unchanged
  ))
